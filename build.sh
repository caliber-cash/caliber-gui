...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/caliber-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    MONEROD_EXEC=caliberd.exe
fi
...
$QMAKE ../caliber-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit

# Copy caliberd to bin folder
