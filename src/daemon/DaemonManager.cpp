...
{
    // prepare command line arguments and pass to caliberd
    ...
    qDebug() << "starting caliberd " + m_caliberd;
    ...
    // Start caliberd
    bool started = m_daemon->startDetached(m_caliberd, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM caliberd.exe");
#else
                QProcess::execute("pkill caliberd");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./caliberd status` returns BUSY when syncing.
...
bool DaemonManager::sendCommand(const QString &cmd, NetworkType::Type nettype, QString &message) const
{
    ...
    p.start(m_caliberd, external_cmd);
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to caliberd
#ifdef Q_OS_WIN
    m_caliberd = QApplication::applicationDirPath() + "/caliberd.exe";
#elif defined(Q_OS_UNIX)
    m_caliberd = QApplication::applicationDirPath() + "/caliberd";
#endif

    if (m_caliberd.length() == 0) {
